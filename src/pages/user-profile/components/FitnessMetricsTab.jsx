...
const FitnessMetricsTab = ({ user, onUpdateMetrics }) => {
  ...
  const handleAddWeight = async () => {
    if (newWeight && !isNaN(newWeight)) {
      ...
      try {
        console.log('📤 Updating weight in Supabase:', newWeight);
        await updateUser(user.id, { weight: Number(newWeight) });
        console.log('✅ Weight updated in Supabase');
      } catch (error) {
  ...
  const handleSaveHeight = async () => {
    if (newHeight && !isNaN(newHeight)) {
      ...
      try {
        console.log('📤 Updating height in Supabase:', newHeight);
        await updateUser(user.id, { height: Number(newHeight) });
        console.log('✅ Height updated in Supabase');
      } catch (error) {
  ...
  const handleSaveAge = async () => {
    if (newAge && !isNaN(newAge)) {
      ...
      try {
        console.log('📤 Updating age in Supabase:', newAge);
        await updateUser(user.id, { age: Number(newAge) });
        console.log('✅ Age updated in Supabase');
      } catch (error) {
